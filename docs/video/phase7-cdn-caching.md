...
| --- | --- | --- | --- | --- |
| HLS segment | `processed/hls/{mediaId}/v720p_000.ts` | `public, max-age=31536000, immutable` | Public | `MEDIA_PUBLIC_BASE_URL` |
...
3. (Optional) Run the dev CDN proxy if you want a caching layer in front of MinIO:
   - `pnpm --filter @app/web tsx infra/cdn/dev-media-proxy.mts`
   - Point `MEDIA_PUBLIC_BASE_URL` (or `MEDIA_CDN_BASE_URL`) to the proxy address you choose (for example: `http://localhost:9100/media`). MinIO already serves media directly at `http://localhost:9000/media-public`, so you can skip the proxy for local playback.
4. Configure environment variables in `apps/web/.env.local` (and worker env) to include:
   - `MEDIA_PUBLIC_BASE_URL` (e.g. `http://localhost:9000/media-public`)
   - `MEDIA_CDN_BASE_URL` (legacy fallback; keep in sync with `MEDIA_PUBLIC_BASE_URL` when set)
...
7. Inspect network requests in the browser DevTools to confirm:
   - HLS manifests and segments load from `MEDIA_PUBLIC_BASE_URL`.
