...
type PayType = (typeof PAY_TYPE_OPTIONS)[number]["value"];
type SortType = (typeof SORT_OPTIONS)[number]["value"];

const PAY_TYPE_LABELS = new Map<PayType, string>(
  PAY_TYPE_OPTIONS.map((option) => [option.value, option.label] satisfies readonly [PayType, string]),
);
const SORT_LABELS = new Map<SortType, string>(
  SORT_OPTIONS.map((option) => [option.value, option.label] satisfies readonly [SortType, string]),
);

function isPayType(value: string): value is PayType {
  return PAY_TYPE_LABELS.has(value as PayType);
}

function isSortType(value: string): value is SortType {
  return SORT_LABELS.has(value as SortType);
}
